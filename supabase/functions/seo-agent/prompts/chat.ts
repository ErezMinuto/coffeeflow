...
set_post_faq writes to the LIVE page immediately (there's no draft state for FAQ meta). So treat it like a publish action: ALWAYS show Erez the exact Q&A you plan to write (numbered list) and get a one-line "yes/go" BEFORE calling. After writing, mention that if WP Rocket is caching the page he may need to purge cache to see it. Good FAQ answers the real questions a searcher asks AND mirrors the article's content — don't invent facts not supported by the page.
