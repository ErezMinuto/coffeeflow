...
ישראלים הם שתייני קפה בין הגבוהים בעולם (צריכה ממוצעת: 2.5-3 כוסות ביום).
התרבות: קפה = חברתי, לא רק תפקודי. בית קפה = מקום לפגישות, עסקים, חופש.
מעבר לאיכות: השוק עבר מנסקפה/אליט → אספרסו → ספשלטי. המעבר מואץ אחרי קורונה (home brewing זינק).
המילה "ספשלטי" — מוכרת בתל אביב ובקרב coffee geeks, פחות בפריפריה. מה שעובד לכולם: "קלוי טרי", "ישירות מהקלייה", "קפה אמיתי".
הצרכן הישראלי חשוד מטבעו — הוא לא מאמין לשיווק. הוא מאמין לחברים, לביקורות, ולעובדות ספציפיות.
גורם WhatsApp: ממליצים על מוצרים בקבוצות וואטסאפ משפחה/חברים — זה הגורם הויראלי הגדול ביותר בישראל.

--- מה מניע רכישה ---
1. "נגמר לי הקפה" — דחיפות מיידית. הקונה לא עושה מחקר — הוא קונה מה שמופיע ראשון בגוגל.
2. "מחפש מתנה" — לפני חגים: ראש השנה, חנוכה, פסח, פורים. קפה = מתנה פרמיום שנחשבת מקורית.
3. "ראיתי ואמרתי 'וואו'" — תוכן אינסטגרם/ריילס שמראה את הקלייה או מקור הפולים. המרה לא מיידית אלא בעוד 3-7 ימים.
4. "מישהו המליץ לי" — WhatsApp/פה לאוזן. הקונה מגיע ישיר לברנד, לא דרך גוגל.

--- תשלומים ו-e-commerce ישראלי ---
• תרבות "תשלומים": ישראלים מצפים לאפשרות של 3-6 תשלומים גם על קנייה של ₪150. תן תשלומים = פחות התלבטות.
• משלוח חינם: סף הכי אפקטיבי לקפה — ₪150-200. מתחתיו — אנשים לא קונים, מעליו — מגדילים הזמנה כדי לעבור את הסף.
• מהירות משלוח: "מגיע תוך 2-3 ימים" = סטנדרט. "מגיע מחר" = יתרון תחרותי משמעותי.
• ביקורות גוגל: קריטיות. ישראלי שמחפש עסק בגוגל — בודק ביקורות לפני שהוא לוחץ.
• ניסיון ראשון: קונה ראשון קונה 250g לנסות. אם הוא אוהב — יחזור ויקנה 500g/1kg.
• מנוי קפה: עדיין מוקדם בישראל אבל גדל. לקוח מנוי שווה 4-5x לקוח חד-פעמי.

--- Google Ads — מה עובד בישראל ---
• תמהיל אפקטיבי: Search (60%) + Shopping (30%) + Retargeting (10%)
• ROAS ריאלי לקפה ספשלטי ישראלי: 3x-5x. מתחת ל-2x = בעיה. מעל 6x = סקייל מיידי.
• CPC ממוצע לקטגוריה: ₪0.80-₪2.50 (נמוך משמעותית מארה"ב/אירופה — הזדמנות)
• כוונת קנייה גבוהה (Exact/Phrase Match): "פולי קפה", "קפה לאספרסו", "קפה ספשלטי", "קנה קפה", "פולי קפה איכותיים"
• כוונת מידע (נמנע לפרסום ממומן): "מה זה ספשלטי", "איך מכינים קפה", "הבדל espresso filter"
• Negative keywords חובה: "קפה נמס", "נספרסו תואם", "קפסולות", "זול", "מתכון", "קפה ערבי", "בוץ"
• שעות שיא: 7-10 בבוקר (לפני עבודה), 21-23 בלילה (קנייה מהספה). הגדל bid adjustment בשעות אלה.
• ימי שכר (5-12 לחודש): שיפור של 20-30% בהמרות — הגדל תקציב בימים אלה.
• קמפיין מתנות: השקע 4-6 שבועות לפני חגים גדולים (ראש השנה, חנוכה). ROI גבוה מאוד.

--- ניתוח ביצועים — אבחנה נכונה ---
• CTR נמוך (<2%): בעיית קריאייטיב — הכותרות לא מדברות לקהל. שכתב.
• CTR גבוה + המרות נמוכות: בעיית landing page — האתר לא ממיר. בדוק מהירות/UX/מחיר.
• ROAS=0: בדוק conversion tracking לפני הכל — לעיתים הבעיה היא טכנית לא שיווקית.
• CPC עולה ו-ROAS יורד: מתחרה חדש נכנס לשוק, או שהקריאייטיב התיישן — חדש מודעות.
• קמפיין ללא המרות 14+ ימים: עצור. אל תמשיך לממן.
• קמפיין עם ROAS>4: הגדל תקציב ב-25-30% בשבוע — לא יותר, כי גוגל צריך זמן לאופטימיזציה.

--- LTV ו-CAC — המספרים לקבלת החלטות ---
• AOV ממוצע לקפה ספשלטי ישראלי: ₪150-280
• CAC ריאלי: ₪80-180 ללקוח חדש (תלוי בתחרות ובמילות המפתח)
• לקוח חוזר: קנייה ממוצעת כל 3-6 שבועות. 10 קניות ב-18 חודשים = ₪1,500-2,800 הכנסה
• מסקנה: שווה לשלם עד ₪200 על לקוח אם הוא ירכוש שוב. אל תחשב ROAS על הרכישה הראשונה בלבד.
• ריטנשיין: לקוח שמזמין פעם שנייה תוך 60 יום — יהיה לקוח לאורך זמן ב-70% מהמקרים.
...
הקהל הישראלי באינסטגרם: 25-45, אורבני, אוהב אוכל ושתייה, גולל בטלפון בדרך לעבודה ולפני שינה.
קפה באינסטגרם = LIFESTYLE, לא מוצר. מה שגורם לו לשמור (save) פוסט: תוכן שגורם לו להרגיש שהוא רוצה את החיים האלה.
מה שגורם לו לרכוש: שילוב של תוכן עם ביקורות + הצעה ברורה + קישור קל לרכישה.
ישראלים שותפים תוכן בוואטסאפ — אם פוסט שלך עובר לקבוצת "קפה" של חברים, זה הגדלה אורגנית מדהימה.

--- תוכן שמביא תוצאות (מה שעובד לקפה בישראל) ---
TOP TIER (saves גבוה, שיתופים, המרות):
  • "behind the scenes" קלייה — האש, הצבע של הפולים שמשתנה, הריח שאפשר כמעט להרגיש
  ...
  • לקוח מרוצה + ציטוט אמיתי — לא שחקן, לא מדומה. אדם אמיתי עם שם
  • תהליך ההכנה — קפה נשפך, קרם אספרסו, לחץ מים. חושני, לא מילות שיווק.

MIDDLE TIER (reach טוב, saves בינוני):
  • "פולי השבוע" — מה קלינו היום ולמה זה מיוחד
  • טיפ קצר — "למה הקפה שלך מר? כנראה..." 30 שניות ריילס
  • הצצה למחסן/קלייה — לא צריך להיות מושלם, צריך להיות אמיתי

BOTTOM TIER (לא להשקיע בו):
  • תמונת מוצר על רקע לבן + מחיר
  • פוסט "קנו עכשיו" ישיר ללא סיפור
  • תוכן כללי על "איך להכין קפה" — כולם עושים את זה

--- פורמטים ועיתוי ---
• ריילס: 3-5x יותר reach מפוסט. תמיד עדיפות. משך אופטימלי: 15-45 שניות.
• קאפשן: בעברית תמיד. קצר בשורה הראשונה (עוצרת גלילה). האשטגים בסוף.
• שעות פרסום: ראשון-חמישי 7:30-9:00 + 20:00-22:00. שישי 8:00-10:00 (פיק).
• תדירות: 4-5 פוסטים בשבוע (מעל זה — ירידה ב-engagement). Stories כל יום.
• האשטגים: 5-10 ממוקדים > 30 כלליים. מיקס: #קפה #ספשלטי #קלייה + English: #specialtycoffee #singleorigin #freshroasted

--- מה מעביר מ-follow לרכישה ---
1. Stories עם link — "השבוע קלינו Ethiopia Guji. לינק בביו לקנות לפני שייגמר"
2. Urgency אמיתי — "נשארו 8 שקיות" (רק אם נכון)
3. Social proof מוכנסת בתוך תוכן — לא פרסומת, תוכן שאגב מזכיר שמישהו אהב
4. DM → מכירה: מי שכותב DM על מוצר — ענה תוך שעה עם קישור ישיר.

--- SEO — מצב השוק ואסטרטגיה ---
הזדמנות: רוב בתי הקלייה הישראלים לא משקיעים ב-SEO. Minuto עם 10+ שנים של היסטוריה — יש Domain Authority טוב.
מילות מפתח גבוהות-נפח-נמוכות-תחרות לדרג: "קפה ספשלטי", "פולי קפה טרי", "בית קלייה ישראל", "קפה מקור יחיד"
מה לכתוב (בלוג):
  • "מה ההבדל בין Ethiopia Yirgacheffe ל-Kenya AA?" — שאלה שאנשים מחפשים, אתה בעל הידע
  • "איך בוחרים קפה לפי שיטת הכנה" — SEO + conversion גבוה (מגיע קהל שמוכן לקנות)
  • "מתנות קפה לחגים" — שאילתות עונתיות עם כוונת קנייה גבוהה
Local SEO: "בית קלייה רחובות" — תחרות כמעט אפסית. אפשר לדרג מהר.
E-E-A-T: גוגל אוהב ידע אמיתי. כתוב כמי שקולה קפה 10 שנים — לא כמי שקרא עליו.
=== סוף ידע מקצועי תוכן ===`;
...
חשוב מאוד — כתוב בעברית ישראלית אמיתית, לא תרגום מאנגלית:
ישראלים מדברים בקיצור, עם ביטחון, בלי לנפח. משפטים קצרים. לא "we believe in freshness" בעברית.

דוגמאות לסגנון (בלבד — אל תעתיק מהן, הן להראות את הטון):
✓ "קלינו ביום רביעי. מגיע אליך ביום ראשון." — עובדות, לא שיווק
✓ "8 פולים. כל שבוע קלייה חדשה." — ספציפי, לא עמום
✓ "ספשלטי ברחובות — ₪59 ומעלה" — מחיר + מיקום = אמין
✓ "Ethiopia Yirgacheffe. פרחוני. חמצמץ. מגיע טרי." — מאפיינים אמיתיים

דוגמאות גרועות — אל תכתוב ככה:
✗ "כל גרם טרי ועם טעם שנשמר" — ועם+שנשמר = תרגום מאנגלית, מוזר בעברית
✗ "לא מדף. קלוי טרי ונשלח." — אל תשתמש בדיוק בדוגמאות שלמעלה, תמצא משהו אמיתי
✗ "טריות מובטחת", "איכות ללא פשרות", "בידול אמיתי" — קלישאות ריקות
✗ "קלויים טרי" — שגיאת הסכמה. "קלויים" רבים זכר → "קלויים טריים"
...
✗ "טריות מובטחת", "איכות ללא פשרות", "בידול אמיתי" — קלישאות ריקות
✗ "קלויים טרי" — שגיאת הסכמה. "קלויים" רבים זכר → "קלויים טריים"
✗ מילים מחוברות ב-ועם/אשר — תרגום מאנגלית. בעברית מדוברת לא מדברים ככה
✗ אל תשתמש בדיוק בדוגמאות הטובות שלמעלה — הן להראות טון, לא לשכפול
✓ שמות מקור כותבים באנגלית: Ethiopia Yirgacheffe, Kenya AA, Brazil Natural
