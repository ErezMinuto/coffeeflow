...
=== כתיבת קופי לGoogle Ads — קופי ישראלי אמיתי ===

גבולות טכניים (ספור לפני שאתה שולח):
• כותרות: עד 30 תווים כולל רווחים ופיסוק. בדרך כלל 3-4 מילים קצרות בלבד.
• תיאורים: עד 90 תווים.

--- מה זה קופי ישראלי אמיתי ---
ישראלי שמחפש בגוגל מקדיש למודעה שניה אחת. הוא חשדן, לא מאמין לשיווק, מאמין לעובדות.
הנוסחה: עובדה ספציפית > תכונה כללית. ניגוד > הסבר. קצר > מלא.
כתוב כמו שחבר מספר, לא כמו שמשווק מוכר.

כותרות שעובדות (אל תעתיק — הן להדגמת הסגנון בלבד):
✓ "קלוי אתמול. אצלך מחר." [21] — עובדה + תועלת. אפס אדג'קטיבים.
✓ "ספשלטי. לא נספרסו." [18] — ניגוד ישיר. מדבר לצרכן שמחפש חלופה.
✓ "330 גרם. קלוי השבוע." [20] — עובדות יבשות. לא צריך יותר.
✓ "Ethiopia Yirgacheffe. ₪69." [26] — מוצר + מחיר. מינימלי. עובד.
✓ "קפה שקלינו. לא קפסולה." [22] — ניגוד, זהות Minuto, ישראלי בלב.

תיאורים שעובדים:
✓ "קלינו ב-6 בבוקר. אתה שותה מחר. זה ההבדל בין קפה לקפה." [53]
✓ "8 זנים שונים. כל שבוע מגוון חדש. 330 גרם ₪59. משלוח חינם מ-₪200." [64]
✓ "פולי קפה ספשלטי מברחובות. קלייה שבועית, מגיע ב-4 ימים." [53]

--- מה אסור לחלוטין ---
✗ "טריות אמיתית" / "איכות אמיתית" — "אמיתית" אחרי שם עצם = קלישאה. NEVER.
✗ "בחר, הזמן, תהנה" וכל CTA בסגנון 3 פעלים — תרגום אמריקאי, לא ישראלי. NEVER.
✗ "Ethiopia Yirgacheffe, Kenya AA, Brazil Natural" בכותרת — זו רשימה, לא קופי, ו-46 תווים. NEVER.
✗ "פרמיום", "בוטיק", "איכותי", "מיוחד", "ייחודי" — כולם אומרים את זה. NEVER.
✗ "ועם", "אשר", "הינו", "מגיע אליך" (→ "מגיע לך") — עברית פורמלית/מתורגמת. NEVER.
✗ "קלויים טרי" — שגיאת הסכמה → "קלויים טריים". NEVER.
✗ כל כותרת עם "קלינו ביום X, מגיעים ביום Y" — תמיד 36+ תווים. NEVER.

הכלל הזהב: אם אפשר לתרגם את הכותרת ל-"Fresh quality premium artisan beans" — היא גרועה.
אם הכותרת אומרת משהו שרק Minuto יכולה לומר, עם עובדה אחת ספציפית — היא טובה.
