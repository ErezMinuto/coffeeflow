...
// Lean Meta Ads expertise — prompt weight kept low to avoid gateway timeout.
// Covers the critical decisions (objective / audience / creative / budget)
// with zero filler. Full playbook was trimmed from ~1500 → ~500 tokens.
const META_ADS_EXPERTISE = `
=== Meta Ads (FB+IG) — בניית קמפיין ===
Objective: Sales (ברירת מחדל, דרוש Pixel) > Catalog Sales (רטרגטינג דינמי) > Traffic (awareness מוצר חדש) > Engagement (Boost פוסט) > Leads (רק אם יש lead magnet).

Audience — מהחזק לחלש:
[1] Custom (חם): Website 90d | Cart 30d | Past Purchasers | IG engagers 365d → רטרגטינג ₪30-50/יום
[2] Lookalike (סקיילינג): 1% LAL of Purchasers > 1% LAL High-AOV > 2-3% LAL Website → ₪80-150/יום
[3] Advantage+ Audience (ברירת מחדל לחדש): רק גבולות גיל/מיקום, Meta מוצאת קהל
[4] Detailed (כשאין data): interests רלוונטיים = Specialty coffee, De'Longhi, Nespresso (שדרוג!), Home barista, V60/Aeropress | למסחרי: Illycaffè, Lavazza, Moka pot, Italian coffee. גיל 28-55, ישראל, עברית+אנגלית. לא "High net worth".
חריגים: לא 18-24 (קפסולות), לא 65+ (לא קונים אונליין).

Placements: Advantage+ Placements (ברירת מחדל). ידני = IG Feed + Reels + Stories + FB Feed. לא Audience Network. IG > FB לקהל ישראלי 25-45.

Creative:
טקסט: Primary ≤125 (המשפט 1 עוצר גלילה) | Headline ≤27 | Desc ≤27 | CTA: Shop Now/Order Now
פורמטים לפי יעילות: Reels (8-15s, הכי זול CPM) > Single Image (אור טבעי, תיאור ברור) > Carousel (מסע 3-5) > Video 15-60s (רק אם יש סיפור). לא תמונת מוצר על רקע לבן.

תקציבים: מינימום ₪30/יום ללמידה. רטרגטינג ₪20-50. LAL ₪80-150. Advantage+ Shopping ₪100-200. כלל: CPA>₪30 ב-3 ימים → עצור. CPA<₪10 → הגדל 20% כל יומיים.

מבנה מומלץ (CBO, קמפיין אחד 3 ad sets):
- Retargeting (Website+Cart): ₪40/יום
- LAL 1% Purchasers: ₪80/יום
- Advantage+ Broad: ₪80/יום
Meta מחלקת אוטומטית. תוך שבוע רואים מנצח.

עיתוי: ראשון 7-10, ג'-ה' 12-14, שישי 7-10, מוצ"ש 20-23. חגים: +50% תקציב שבוע לפני. ר"ה+פסח = פיקים.
=== סוף ===`;
