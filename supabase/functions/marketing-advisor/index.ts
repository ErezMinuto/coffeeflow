 ...
 *   organic_content       — RETIRED 2026-05-27 (replaced by organic-orchestrator; see archive/)
 *
 * POST body: { "trigger": "manual"|"cron", "agent": "google_ads_growth"|"google_ads_efficiency"|"all" }
 *
...
// Full Meta Ads expertise — used ONLY by the on-demand build_meta_campaign
// endpoint, not by the strategist agents (they'd exceed Supabase's 150s
// gateway with this in their prompt). On-demand calls are isolated and
// single-focused, so the full playbook fits comfortably.
const META_ADS_EXPERTISE = `
=== בניית קמפיין Meta Ads (Facebook + Instagram) — מדריך מלא ===

--- בחירת Objective ---
• Sales (Conversions): ברירת המחדל למינוטו. ההמרה = רכישה/עגלה. דרוש Pixel פעיל.
• Catalog Sales (Advantage+ Shopping): המלצה חמה אם יש Pixel + פיד WooCommerce מחובר ל-Meta Commerce. רטרגטינג דינמי ומוצרים רלוונטיים.
• Traffic: awareness מוקדם של מוצר חדש או בלוג פוסט. כמעט תמיד פחות יעיל מ-Sales.
• Engagement: Boost פוסט אורגני חזק. לא קמפיין חדש.
• Leads: רק אם יש lead magnet (eBook, מדריך). למינוטו — משני.

--- בחירת Audience (קהל) ---
[1] Custom Audiences (חם — הכי ממיר):
  • Website Visitors 90d (Pixel) | Cart Abandoners 30d | Past Purchasers (CRM) | Video viewers 75%+ 180d | IG engagers 365d
  • רטרגטינג ₪30-50/יום. ROAS גבוה.

[2] Lookalike (סקיילינג):
  • 1% LAL of Past Purchasers = 100-200K בישראל, הכי קרוב ללקוחות
  • 1% LAL of High-AOV (>₪250, מינ' 100 לקוחות source)
  • 2-3% LAL of Website Visitors לרחב יותר

[3] Advantage+ Audience (המלצת Meta לחדש/scaling):
  רק גבולות גיל/מיקום/שפה, Meta מוצאת קהל דרך AI. לרוב עולה על Detailed במודעות חדשות.

[4] Detailed Targeting (כשאין data):
  לספשלטי (חובבים): Specialty coffee, Third wave coffee, Coffee roasting, Espresso, Home barista, V60, Aeropress, French press, De'Longhi, Nespresso (!שדרוג), Coffee culture
  למסחרי (סופר): Coffee machine (home), Bialetti, Moka pot, Stovetop espresso, Italian coffee, Illycaffè, Lavazza, Home coffee
  התנהגויות: Engaged shoppers + Online shoppers. לא "High net worth".
  גיל 28-55 | ישראל | עברית+אנגלית.

חריגים: לא 18-24 (קפסולות), לא 65+ (לא קונים אונליין), לא Low-income clusters.

--- Placements ---
• ברירת מחדל: Advantage+ Placements (Meta בוחרת: Feed, Reels, Stories, AN).
• ידני: IG Feed + IG Reels + IG Stories + FB Feed. לא Audience Network.
• IG > FB לגילאי 25-45 בישראל (שינוי 2023-24).

--- Creative ---
טקסט: Primary ≤125 (משפט 1 עוצר גלילה) | Headline ≤27 | Desc ≤27 | CTA: Shop Now | Order Now
פורמטים (לפי יעילות): Reels (8-15s, CPM הכי זול) > Single Image (אור טבעי) > Carousel (מסע 3-5) > Video 15-60s (רק אם יש סיפור).
לא: תמונת מוצר על רקע לבן.

🚫 **אסור לחלוטין בטקסט/ויז'ואל של מודעה:**
• שמות מתחרים (Lavazza, Illy, Hausbrandt, Nespresso, נחת, Jera, אגרו, Mauro, Bristot, Kimbo, Segafredo) — trademark + חוק הגנת הצרכן בישראל + Meta policy. מודעה תידחה או תגרור תלונה.
• טענות על מתחרים ("הפולים של X נקלו לפני 4 חודשים") — גם אם נכון, זה השמצה. אסור.
• לוגואים של מותגים אחרים — אפילו מיטושטש/ניטרלי קרוב מדי מזוהה.
✓ במקום לנקוב בשם: "הפולים מהסופר" / "השקית הקיימת שלכם" / "קפה מסחרי" — תקיפה קטגורית, לא trademark.
✓ טריק יעיל: **גורם לצופה להסתכל על עצמו** — "בשקית שלך יש תאריך קלייה?" במקום "בשקית של X אין".

🎯 **כללי הפקה לקריאייטיב שעובד ב-Reels (1.5 שניות עד skip):**
1. **Hook = payoff frame, לא setup.** הפריים הראשון חייב להיות הפואנטה (בסוד: "למה לעצור?"). אל תפתח עם "הפולים של המתחרה" — זה setup משמים. פתח עם קלוזאפ של הפולים שלנו + הטקסט הכי חזק.
2. **משפט אחד, לא ארבעה.** טקסט על המסך = צופה קורא במקום להסתכל. בחר משפט אחד שנושא את כל המודעה ("נקלה הבוקר. אצלך מחר.").
3. **תן חוויה חושית.** קפה טרי הוא ריח, אדים, טעם ראשון — לא מפרט. הראה יד שיוצקת, פנים שנרגעים, אור בוקר. **אל תתן דף מוצר.**
4. **CTA לא רק בסוף.** מותג/לוגו חייב להיות מרגע 3, לא רק בשנייה 10 — רוב הצופים נוטשים לפני 8.
5. **"תגרום להם להסתכל" > "תגיד להם מה."** מודעה שגורמת לצופה להרים את שקית הקפה שלו ולחפש תאריך = 10× engagement של מודעה שאומרת "שלנו טריים".
6. ⚠️ אם הברייף שלך נראה כמו דף מוצר עם מוזיקה — זה לא ברייף שיווקי, זה מפרט. שנה אותו.

--- תקציבים ---
• מינימום ₪30/יום ללמידה. רטרגטינג ₪20-50. LAL ₪80-150. Advantage+ Shopping ₪100-200.
• כלל: CPA>₪30 תוך 3 ימים → עצור. CPA<₪10 → הגדל 20% כל יומיים.

--- מבנה מומלץ (CBO, 3 ad sets): ---
A: Retargeting (Website+Cart): ₪40/יום
B: LAL 1% Purchasers: ₪80/יום
C: Advantage+ Broad: ₪80/יום
Meta מחלקת אוטומטית; תוך שבוע מנצח ברור.

--- עיתוי ---
ראשון 7-10 | ג'-ה' 12-14 | שישי 7-10 | מוצ"ש 20-23. חגים: +50% תקציב שבוע לפני. ר"ה+פסח = פיקים.
=== סוף ===`;

// ── Playbook 2026 — distilled from external research on DTC coffee, IG algo,
// Meta Advantage+, Google Ads best practices, and Israeli specialty landscape.
// Injected into every strategist/doctor prompt as authoritative guidance.
const PLAYBOOK_2026 = `
=== ספר המשחקים של 2026 — best practices מבוססי מחקר ===

--- שלב 1: נתח את הפאנל, לא את ה-KPI המבודד ---
קמפיין טוב = שרשרת 5 שלבים שכולם עובדים. אבחן את הצוואר בקבוק הספציפי:
• שלב 1 Impressions — בעיה: impression_share < 20%. פעולה: הגדל תקציב/bid, לא קריאייטיב.
• שלב 2 Clicks — בעיה: CTR < 2%. פעולה: שכתב כותרות/תיאורים, לא תקציב.
• שלב 3 Landing — בעיה: CTR טוב אבל real_orders=0. פעולה: תקן עמוד נחיתה, לא קריאייטיב.
• שלב 4 Cart — בעיה: add-to-cart גבוה, purchase נמוך. פעולה: תקן מחיר/משלוח/תיאור מוצר.
• שלב 5 Purchase — בעיה: ROAS<1×. פעולה: תקהל אחר או מוצר במחיר גבוה יותר.
**כלל**: אל תמליץ פעולה שלא מטפלת בצוואר הבקבוק הספציפי.

--- Google Ads 2026 — best practices ---
• **הפרדת ברנד מ-non-brand**: קמפיינים נפרדים עם ROAS targets נפרדים. ברנד CPA ₪5-15, ספשלטי ₪40-80.
• **Single-Themed Ad Groups (STAGs)**: ad group אחד לכל נושא מובחן. לא "כל הקפה" בקמפיין אחד.
• **Match type tiers**: EXACT [הכוונה גבוהה] + PHRASE "וריאציות" + BROAD (רק עם Smart Bidding).
• **Target ROAS**: דורש 15+ המרות ב-30 יום. פחות = הישאר Maximize Conversions או Max Conversion Value.
• **Search Partners + Display Network**: כבה על קמפייני Search — איכות לקוחות ירודה.
• **Performance Max**: דורש ₪40-80/יום מינימום ללמידה תקינה. פחות = stuck in learning.
• **Final URL expansion**: כבה ב-PMax אם יש קטלוג מגוון — מוביל traffic לדפים לא רלוונטיים.

--- Meta Ads 2026 — best practices ---
• **מבנה פאנל**:
  - TOF (prospecting): 60-70% תקציב, קהלים קרים, יעד = הודעות ראשונות
  - MOF (retargeting 7-30 ימים): 15-20% תקציב, תוכן שעונה על התנגדות
  - BOF (cart abandoners): 10% תקציב, תזכורות + הצעות
• **Conversion event = Purchase בלבד**. אל תאפטם על Add to Cart או View Content.
• **Creative fatigue triggers** (כל אחד = זמן להחליף קריאייטיב):
  - Frequency > 2.5 ב-TOF או > 4-6 ב-BOF
  - CTR נפל 20%+ שבוע-על-שבוע
  - CPM עלה 15%+ ללא שינוי טירגוט
• **Creative diversity > volume**: 3 זוויות × 2 פורמטים > 10 וריאציות של אותו רעיון.
• **Video**: 3-15 שניות לקר, 60-180 שניות לחם. Explainer 3-5 דקות = BOF בלבד.
• **Advantage+ Shopping (ASC)**: דורש pixel חזק + 50+ purchases/week. אם לא — שלב TOF ידני.

--- Instagram 2026 — אלגוריתם חדש ---
**הגילוי החשוב של 2026**: saves > likes במשקל האלגוריתמי. ריל עם 3,000 likes + 400 saves מנצח ריל עם 10,000 likes + 12 saves.
• **3 השניות הראשונות** = 60-70% מ-watch completion. ללא hook חזק — הריל נקבר.
• **Resonance metric חדש**: זמן שאנשים משהים על פוסט + actions (save/share). עדיפות על פני likes.
• **מיקס אופטימלי**: 3-4 ריילס + 2-3 קרוזלים + 1-2 סטטיים לשבוע.
• **content save-bait** (המוכשר ביותר):
  - מדריכי how-to ("5 טעויות באספרסו")
  - טבלאות/רפרנסים (יחסי V60, טמפרטורות)
  - תיקון בעיות ("איך לנקות מטחנה")
  - מפות טעם (אתיופי vs קולומביאני vs ברזילאי)
• **content share-bait**:
  - קונטרה/חשיפה ("זה מה שהסופר לא מספר")
  - הומור פנימי של החובבים
  - אתגר/בלינד טייסט
  - טריוויה מפתיעה

--- Email retention (coffee-specific) ---
• **מחזור רכישה טיפוסי לפולי קפה**: 3-6 שבועות (שקית 250g מספיקה ~3 שבועות שותים).
• **Replenishment flow**: תזכורת ב-70% של המחזור (יום 14-18 אחרי קנייה).
• **Winback trigger**: 45 ימים ללא פעילות.
• **Winback flow**: מקסימום 3 מיילים. מגע קל → ערך → הצעה אחרונה. לא להציע הנחה במייל #1.
• **תוצאה צפויה**: שחזור 3-8% מהלקוחות הרדומים.
• **Sunset flow**: מי שלא הגיב לwinback → הפחת תדירות ל-חודשי, אל תסיר מ-מסד.

--- נוף תחרותי מקומי (Israel specialty 2026) ---
• **Tel Aviv boutique roasters**: Nahat (הכי מזוהה, מוכר דרך חנות+משלוח), Cafelix, Mae (משפחתי, 4 סניפים), Jera (קפה עם פוקוס על לאטה).
• **Jerusalem**: Roasters (3 סניפים + משלוח חינם מ-₪199), Sybaris, Birma.
• **קלייה ייעודית**: Portofino (blend focus), Kilimanjaro (online-first).
• **מחיר ממוצע לשקית 250g ספשלטי**: ₪80-120.
• **חולשות נפוצות של מתחרים**:
  - רוב אין SCA score על השקית (מינוטו יכול לפרסם 83+).
  - רוב אין תאריך קלייה בולט (מינוטו כן).
  - רוב פועלים בתל אביב (מינוטו ברחובות = תכונה לא חולשה — "מעבר לבאלגן").
  - מעטים עושים B2B בצורה מאורגנת (מינוטו מוכר Marzocco + commercial).

--- heuristics להחלטות ---
• **AOV מינוטו**: ~₪150
• **Target CPA**: ₪30-60 (ספשלטי), ₪15-30 (ברנד), ₪50-80 (מסחרי-B2B)
• **Target ROAS**: 2.5-3× בריא; 1.5-2× מקובל; <1.5 = הפסד.
• **Kill rules במספרים ₪**:
  - קמפיין חדש: עצור אחרי ₪200 ללא purchase (לא "אם CPA גבוה")
  - קמפיין ותיק: עצור אם 5 ימים רצופים מעל 2× target CPA
• **Scale rules**:
  - ROAS 3+ ×2 שבועות → הגדל תקציב 20-30% (לא 100% — Meta/Google learning יתאפס)
  - ROAS 2-3 → שמור, אל תגע
  - ROAS 1.5-2 → בדוק יציבות, אל תגדיל

--- timing ישראלי ---
• שבוע עסקי: א'-ה' (5 ימים). קמעונאות: א'-ו' (6 ימים). פיקי קנייה: א' 7-10, ג'-ה' 12-14, שישי 7-10.
• **חגים עיקריים לתזמון קמפיינים**:
  - ר"ה (ספטמבר): +50% תקציב שבועיים לפני
  - יום כיפור: השבת קמפיינים 24 שעות
  - חנוכה: מתנות — ₪200-400 bundle
  - ט"ו בשבט: תוכן על מוצא ירוק, חקלאות
  - פסח: פריחה — משלוח לפני יציאת העיר, BBQ של חול המועד
  - **יום העצמאות**: BBQ + קפה בבוקר = זיווג קלאסי. קונים מחר, מגיע לפני החג.
...
=== 🎯 שלב 0.5: אני מנהל קפיטל, לא גנרטור רעיונות ===
הבעלים יכול לחשוב על רעיונות בעצמו. מה הוא לא יכול לעשות בעצמו? חשבון הוגן של ROI, עלות הזדמנות, וסיכון.
לכן **כל** המלצה שכרוכה בהוצאה חייבת לכלול:
1. **מספר צפוי (range)**: "אם זה עובד, מביא ₪X-Y בהכנסות תוך Z שבועות." לא "צפוי להגדיל מכירות".
2. **מקור המימון**: מאיפה ה-₪? אם אני מבקש ₪80/יום חדשים — חובה לומר מה לקצץ בשביל זה. "לעצור קמפיין X חוסך ₪Y ליום → זה מממן את הקמפיין החדש." לא "להוסיף עוד תקציב".
3. **Kill criteria במספרים ₪, לא באחוזים**: "אחרי ₪240 הוצאה ו-0 המרות — עצור". לא "CPA גבוה → עצור".
4. **Staged rollout**: אסור "תתחיל עם ₪80/יום". חובה 3 שלבים — תקציב קטן כדי לבדוק, הגדל רק אם משהו עבד, עם תנאים מספריים ברורים.
5. **Confidence tier**: כל המלצה חייבת תיוג אחד מהשלושה:
   - **PROVEN**: יש לנו נתון היסטורי שזה עבד (בקמפיין קיים / לקוחות קיימים)
   - **LIKELY**: לא ניסינו, אבל יש signal חזק מהמחקר (competitor מצליח / search volume אמיתי)
   - **SPECULATIVE**: רעיון נועז בלי ולידציה — אסור להוציא כאן מעל ₪X בלי לראות חצי-אות חיובי

⚠️ **שליש לפחות** מהרעיונות חייב להיות **PROVEN** או **LIKELY** — לא כולם speculative. אם הכל speculative זו הימור, לא ניהול.
⚠️ אם אין לך את המספר הספציפי — **תגיד שאתה לא יודע** ("Confidence: speculative. אין לי נתון לתמוך בטווח ההכנסות — לבחון אחרי phase 1"). **אסור לזייף מספרים**.
...
🚫 **רשימה שחורה של מילות מפתח מובנות-מאליהן — אסור להחזיר אותן בשדה keywords[] של campaigns_to_create (אפשר להזכיר באסטרטגיה, אבל לא לבחור לקמפיין):**
...
סיבה: אלה הביטויים שכל סוכן סטנדרטי היה בוחר. התחרות עליהם מטורפת, ה-CPC גבוה, אין ייחודיות. אם הקמפיין שלך בנוי עליהם — אתה מביא תוצר גנרי במקום אסטרטגיה.

✅ **חובה מוחלטת**: בסעיף "🔍 מילות מפתח חדשות (גילוי אוטומטי)" במחקר השוק יש רשימה של ביטויי long-tail שנתגלו אוטומטית (שאלות, בעיות, מצבים). **לפחות 4 מתוך 7 מילות המפתח בכל קמפיין חדש חייבות להילקח מהרשימה הזו**, לא מרשימה גנרית. אם הרשימה מכילה למשל "למה הקפה שלי יוצא מר" — זו מילת מפתח מצוינת עם 40 מפרסמים, והיא גם ייחודית. זה המטרה של הגילוי — השתמש בו.
...
=== 🎯 אני מנהל קפיטל ל-90 ימים, לא גנרטור רעיונות ===
הבעלים צריך ממני החלטת הון אחראית, לא רשימת משאלות. כל מהלך של הוצאה חייב:
1. **expected 90-day revenue range** (low/likely/high) + projected ROAS — לא "זה יגדיל מכירות".
2. **מקור מימון מוגדר**: מאיפה הכסף? לקצץ מ-X, לעצור Y, או להעביר מ-Z. **אסור "להוסיף עוד תקציב" בלי לומר מה מקטינים.**
3. **risk-adjusted ROI**: אחרי הורדת סבירות כישלון (25-40%), כמה באמת ציפייה ריאלית? אם likely=₪50K אז risk-adjusted=₪30K. תן את המספר הזה.
4. **Confidence tier** לכל מהלך: **PROVEN** (יש data היסטורי), **LIKELY** (signal חזק), **SPECULATIVE** (הימור נועז — cap מוגדר).
5. **No-regret moves** (חובה ≥1): פעולה עם downside נמוך ו-upside ברור — לא כולם speculative. אם כולם speculative זה הימור.

⚠️ **אסור לזייף מספרים**. אם אין לך data היסטורי — תגיד "Confidence: speculative, revenue range לא אמין, לבחון אחרי חודש 1". Better לומר "אני לא יודע" מאשר מספר מומצא.
...
async function runBlogWriterAgent(params: {
  ...
  // Bucket equipment by category via name pattern matching.
...
serve(withCors(async (req) => {
  ...
  // ── BUILD META CAMPAIGN — on-demand full spec from a strategist idea ──────
  // Frontend passes a campaign idea (from meta_campaign_ideas[] in a
  // strategist report) and optional budget/audience overrides. Returns a
  // complete implementation spec the owner can paste into Meta Ads Manager.
  // Isolated from the strategists so we can afford the full META_ADS_EXPERTISE
  // in the prompt without hitting the 150s gateway.
  if (body.agent === "build_meta_campaign") {
    const { idea, audience_lens, daily_budget_ils, duration_days, objective_override } = body as any;
    if (!idea || typeof idea !== "string") {
      return new Response(JSON.stringify({ error: "idea (string) is required" }),
        { status: 400, headers: { ...CORS, "Content-Type": "application/json" } });
    }
    const sysPrompt = `${BUSINESS_BRIEF}\n${META_ADS_EXPERTISE}\n
אתה Meta Ads specialist. קיבלת רעיון לקמפיין ואתה בונה ממנו מפרט יישום מלא.
החזר JSON בלבד במבנה הבא. כל שדה חייב להיות מלא — זה מה שהבעלים יעתיק ל-Ads Manager:
{
  "campaign_name": "שם הקמפיין כפי שייראה ב-Ads Manager",
  "objective": "Sales|Catalog Sales|Traffic|Engagement|Leads",
  "daily_budget_ils": 80,
  "duration_days": 14,
  "audience": {
    "type": "advantage_plus|detailed|custom|lookalike|retargeting",
    "definition_step_by_step": [
      "צעד 1: Ads Manager → Audiences → Create → ...",
      "צעד 2: ..."
    ],
    "age_range": "28-55",
    "geo": "ישראל",
    "languages": ["עברית", "אנגלית"],
    "interests_or_behaviors": ["Specialty coffee", "De'Longhi"],
    "custom_or_lookalike_source": "Website 90d | 1% LAL of Past Purchasers — או null אם לא רלוונטי",
    "exclusions": ["קונים 30 יום האחרונים", "Unsubscribed"],
    "why_this_audience": "הסבר למה הקהל הזה לרעיון הזה"
  },
  "placements": "Advantage+ Placements | IG Feed+Reels+Stories+FB Feed",
  "creative": {
    "format": "reel|single_image|carousel|video",
    "visual_brief": "מה לצלם/לעצב — תיאור מפורט מספיק למעצב/צלם",
    "primary_text": "גוף המודעה — ≤125 תווים, המשפט הראשון עוצר גלילה",
    "headline": "כותרת — ≤27 תווים",
    "description": "תיאור link preview — ≤27 תווים",
    "cta_button": "Shop Now | Order Now | Learn More",
    "alternate_creative_variants": [
      "וריאנט 2 — כותרת + גוף שונה לבדיקת A/B",
      "וריאנט 3"
    ]
  },
  "tracking": {
  ...
  "success_metrics": {
    ...
    "kill_threshold_cpa_ils": 30,
    "scale_threshold_cpa_ils": 10
  },
  "notes": "כל דבר חשוב שהבעלים צריך לדעת לפני שמפעיל"
}
ענה אך ורק ב-JSON תקין. אל תוסיף טקסט לפני או אחרי.`;

    const userMsg = `רעיון קמפיין: "${idea}"
עדשת קהל מועדפת: ${audience_lens ?? "auto"}
תקציב יומי ₪: ${daily_budget_ils ?? 80}
משך בימים: ${duration_days ?? 14}
${objective_override ? `Objective מועדף: ${objective_override}` : ""}
...
הפק ספריית פטרנים מכל הציטוטים האלה. צפוי 15-40 פטרנים תלוי בגיוון.`;
  ...
  if (body.agent === "unified_marketing_plan") {
    // TRIGGER MODE — the analysis is a ~85s Claude call, too long for a
    // synchronous browser request (gateway timeout). Mirror the strategist
    // pattern: mark running, fire a non-awaited self-invoke that does the work
    // and writes to advisor_reports, and return 202 immediately. The frontend
    // polls advisor_reports (agent_type='unified_plan') for the result.
    if ((body as any)._run !== true) {
      await upsertReport(supabase, "unified_plan", weekStart, { status: "running", report: null, error_msg: null });
      const selfUrl = `${SUPA_URL}/functions/v1/marketing-advisor`;
      fetch(selfUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${SUPA_KEY}` },
        body: JSON.stringify({ agent: "unified_marketing_plan", week_start: weekStart, _run: true }),
      }).catch((e) => console.error("[unified_plan] fire error:", e?.message));
      return new Response(JSON.stringify({ started: true, week_start: weekStart }),
        { status: 202, headers: { ...CORS, "Content-Type": "application/json" } });
    }

    // WORKER MODE (_run:true) — does the heavy analysis; result → advisor_reports.
    try {
...
${dataSection}`;
      ...
      if (!plan) {
        await upsertReport(supabase, "unified_plan", weekStart, { status: "error", error_msg: "could not parse plan JSON", report: null });
      ...
      await upsertReport(supabase, "unified_plan", weekStart, { status: "error", error_msg: err?.message ?? "unknown error", report: null });
      ...
      const tools = [{
        ...
        input_schema: {
          ...
          properties: {
            ...
            destination: { type: "string", enum: ["website", "whatsapp"], description: "website (default) = ad sends to landing_page_url. whatsapp = Click-to-WhatsApp — tapping opens a WhatsApp chat via the Page (no URL); use for conversation/lead campaigns. IMPORTANT: WhatsApp works ONLY with a FRESH image (image_url), NOT existing_post — Meta's API can't put a WhatsApp CTA on an existing organic reel. To use a reel's look, describe a fresh creative instead." },
          },
...
`;
  ...
  // organic_content removed from BOTH arrays 2026-05-27 (retired agent;
  // see organic-orchestrator). Email digest now fires when both ad
  // strategists complete (was: 3 agents).
  const NEW_AGENTS = ["strategist_aggressive", "strategist_precise"];
  const OLD_AGENTS = ["google_ads_growth", "google_ads_efficiency"];
  ...
  // ── SINGLE AGENT MODE ────────────────────────────────────────────────────────
  //
  // RETIRED agents: explicit 410 with a helpful pointer to the replacement.
  // Caught BEFORE the generic Unknown-agent 400 so operators see the
  // 'use this instead' message rather than a confusing 'unknown' error.
  if (agentArg === "organic_content") {
    const retiredMsg = "organic_content agent is RETIRED 2026-05-27. Use organic-orchestrator (sidebar: סוכן אורגני, route: /admin/seo-agent) instead. Code archived in archive/2026-05-27-organic-content-removal/.";
