...
const ADS_EXPERTISE = `
=== ידע מקצועי: שוק הקפה הישראלי + Google Ads ===

התנהגות צרכני קפה ספשלטי בישראל:
• קהל יעד: 28–50, משכילים, עירוניים, מוכנים לשלם פרמיום אבל רוצים להרגיש שמגיע להם
• מסלול רכישה: גוגל → אתר → מסתכלים על המוצר → נוטשים → חוזרים אחרי 2-7 ימים → קונים
• ריטנשיין: לקוח חוזר שווה 5-8x יותר מלקוח חדש. הזמנה שנייה ב-60 יום = סימן לנאמנות
• טריגרים לרכישה: "נגמר לי הקפה" (דחיפות), "מחפש מתנה" (פסח/חנוכה/יום הולדת), "ראיתי פוסט ורציתי לנסות"
• ימים חזקים לרכישות: ראשון-שלישי בבוקר (7-10). שישי — נמוך (שבת אורבת)
• שעות שיא חיפוש קפה: 7-10 בבוקר, 21-23 בלילה
• ימי שכר (סביב ה-10 לחודש): שיפור משמעותי בהמרות

Google Ads — מה עובד לקפה ספשלטי ישראלי:
• ROAS יעד ריאלי לקפה ספשלטי e-commerce: 3x–5x. מתחת ל-2x = בעיה. מעל 6x = סקייל.
• מילות מפתח עם כוונת קנייה גבוהה: "פולי קפה", "קפה ספשלטי", "קפה מקור יחיד", "קנה קפה"
• מילות מפתח מידעניות (CVR נמוך): "מה זה ספשלטי", "איך לטחון קפה", "הבדל espresso filter"
• Search > Performance Max לקפה ספשלטי — קהל נישה שיודע מה הוא מחפש
• Exact Match + Phrase Match עדיפים על Broad לקהל נישה
• Negative keywords קריטיות: "קפה נמס", "נספרסו תואם", "קפה זול", "חינם"
• Ad Strength: Poor/Average = צריך לשכתב. הוסף כמה שיותר כותרות ייחודיות
• כותרת עם מחיר ("מ-₪59") מגדילה CTR ב-15-25% לקהל price-sensitive
• כותרת עם מיקום ("ברחובות", "משלוח לכל הארץ") מגדילה רלוונטיות

ניתוח ביצועים — מה לבדוק:
• CPC גבוה + המרות נמוכות = בעיית landing page, לא מילות מפתח
• CTR נמוך (מתחת ל-3%) = בעיית קריאייטיב/רלוונטיות
• ROAS 0 כמה שבועות → עצור ובדוק tracking (Pixel, conversion tags)
• קמפיין עם המרות → הגדל תקציב בהדרגה, 20-30% בשבוע
• קמפיין ללא המרות 2+ שבועות → עצור, אל תמשיך לשלם

e-commerce קפה — מדדים להתייחסות:
• AOV (ערך הזמנה ממוצע) טוב לקפה ספשלטי: ₪150-300
• CAC (עלות רכישת לקוח) ריאלית: ₪80-200 לקוח חדש
• LTV (ערך לכל חיים): לקוח נאמן = 12-20 הזמנות ב-3 שנים = ₪1,800-4,000
• כלומר: שווה לשלם עד ₪200 על לקוח חדש אם הוא יחזור
=== סוף ידע מקצועי ===`;

// ── Deep Organic/Instagram Expertise ─────────────────────────────────────────
const ORGANIC_EXPERTISE = `
=== ידע מקצועי: Instagram + SEO לקפה בישראל ===

Instagram — התנהגות קהל קפה ישראלי:
• ריילס > פוסטים: ריילס מקבלים 3-5x יותר חשיפה. עדיפות ברורה.
• שעות פרסום אופטימליות: ראשון-חמישי: 7:30-9:00 בבוקר (בדרך לעבודה) + 20:00-22:00
• שישי בבוקר: פיק — אנשים מחפשים קפה לשבת. תוכן של שישי צריך להיות ב-8-10 בבוקר
• תוכן שמבצע טוב לקפה: "behind the scenes" קלייה, אחמד/origin stories, השוואות טעמים
• תוכן שלא מבצע: מוצרים על רקע לבן, מחיר בלבד, "קנו עכשיו" ישיר
• האשטגים אפקטיביים לישראל: #קפה #ספשלטי #בירהקלייה #קפהישראלי + אנגלית: #specialtycoffee #singleorigin
• Stories → Highlights: שים ביקורות לקוחות, תהליך הקלייה, מגוון הפולים

SEO — מצב שוק קפה ספשלטי בישראל:
• תחרות: נמוכה-בינונית. רוב בתי הקלייה הישראלים חלשים ב-SEO — הזדמנות!
• מילות מפתח עם פוטנציאל גבוה ותחרות נמוכה: "קפה ספשלטי ישראל", "בית קלייה קפה", "פולי קפה טרי משלוח"
• מה כדאי לכתוב (בלוג/דפים): מדריכים ("איך לבחור קפה ספשלטי"), origin stories (Ethiopia Yirgacheffe — מה מיוחד?), מתכוני הכנה
• E-E-A-T לקפה: גוגל מעריך expertise אמיתי. פוסטי בלוג שמראים ידע אמיתי מהקלייה — לא AI fluff
• Local SEO: "בית קלייה רחובות", "קפה ספשלטי רחובות" — אפשר לדרג מהר
• Core Web Vitals: מהירות אתר = גורם דירוג. אם האתר איטי — זה חוסם SEO
=== סוף ידע מקצועי ===`;
