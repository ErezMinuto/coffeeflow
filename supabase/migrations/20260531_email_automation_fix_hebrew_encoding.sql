-- ENCODING REPAIR + DYNAMIC CART (2026-05-31): re-sync abandoned_cart_1 +
-- abandoned_cart_2 template bodies from source AND add the {{ cart_items_html }}
-- placeholder so the email lists what the customer left in their cart.
--
-- Encoding background: the original 20260511 INSERT was applied to prod via a
-- non-UTF-8 client (psql with wrong client_encoding), which stored the Hebrew
-- subject/body as mojibake — test-send rendered gibberish. This file differs
-- from 20260511 in two ways: (1) ON CONFLICT now DO UPDATE the text columns
-- instead of DO NOTHING, so correct UTF-8 overwrites the corrupted rows;
-- (2) a {{ cart_items_html }} placeholder sits above the "back to cart" button.
--
-- IMPORTANT: in prod this content was written via the Supabase REST API (curl,
-- UTF-8 over HTTP) to dodge the clipboard double-encoding that corrupts pastes
-- into the SQL editor. This file is the committed source of truth; if you ever
-- run it by hand, use the REST path or a verified-UTF-8 client, NOT a paste.
--
-- {{ cart_items_html }} is rendered by the scheduler's renderCartItemsHtml():
-- real line items for live sends, sample items for test-sends, '' when empty.
--
