...
export default function RoastingList({ data, originsDb, roastsDb, showToast }) {
  ...
  const toggleCheck = (id) => {
    ...
    if (!amounts[id]) setAmounts(prev => ({ ...prev, [id]: DEFAULT_ROAST_KG }));
  };

  const confirmRoasting = async () => {
    const toRoast = roastingNeeded.filter(o => checked[o.id]);
    if (toRoast.length === 0) { showToast('לא נבחרו זנים לקלייה', 'warning'); return; }
    ...
    for (const origin of toRoast) {
      ...
      if (newGreenStock < 0) { showToast(`אין מספיק מלאי ירוק ל${origin.name}`, 'error'); continue; }

      try {
        await originsDb.update(origin.id, { roasted_stock: newRoastedStock, stock: newGreenStock });
        await supabase.from('roasts').insert({
          origin_id: origin.id, green_weight: greenKg,
          roasted_weight: parseFloat(roastedKg.toFixed(2)),
    ...
    setSaving(false);
    setChecked({}); setAmounts({}); setNotes({});
    ...
    <div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <input type="checkbox" checked={isChecked} onChange={() => toggleCheck(origin.id)} onClick={e => e.stopPropagation()} style={{ width: '20px', height: '20px', cursor: 'pointer' }} />
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #D1FAE5' }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                        <div className="form-group" style={{ margin: 0 }}>
                          ...
                          <input type="number" step="0.5" value={amounts[origin.id] || DEFAULT_ROAST_KG} onChange={e => setAmounts(prev => ({ ...prev, [origin.id]: e.target.value }))} style={{ width: '120px' }} />
                        </div>
                        ...
                        <div className="form-group" style={{ margin: 0, flex: 1 }}>
                          ...
                          <input type="text" placeholder="למשל: דלי #1234" value={notes[origin.id] || ''} onChange={e => setNotes(prev => ({ ...prev, [origin.id]: e.target.value }))} />
                        </div>
                      </div>
                    </div>
                  )}
      ...
      <div style={{ marginTop: '2rem', padding: '1rem', background: '#FFF9F0', borderRadius: '8px', border: '1px solid #F4E8D8' }}>
        <p style={{ fontSize: '0.875rem', color: '#666', margin: 0 }}>
          💡 כדי שרשימה זו תעבוד נכון, הגדר <strong>ממוצע מכירות יומי</strong> לכל זן בדף הזנים (לחיצה על ✏️).
        </p>
      </div>
    </div>
  );
